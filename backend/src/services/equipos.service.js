...
async function borrarImagenSiExiste(Imagen) {
    ...
    try {
        ...
        // Si el archivo no existe en disco no es un error para el usuario.
    }
}

async function listEquipos() {
    const [rows] = await pool.query('SELECT * FROM equipos ORDER BY id_equipos DESC');
    return rows;
}

async function getEquipoById(id_equipo) {
    const [rows] = await pool.query('SELECT * FROM equipos WHERE id_equipos = ?',
        [id_equipo]
    );
...
module.exports = {
    listEquipos, getEquipoById, createEquipo, updateEquipo,
    deleteEquipo
};